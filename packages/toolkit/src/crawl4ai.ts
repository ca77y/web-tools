 ...
 * `{ type, params }` caller envelope. Returns `undefined` when `value` is
 * not a config-shaped object (absent, or not a plain object) so a caller
 * can distinguish "no config" from "empty config".
 */
