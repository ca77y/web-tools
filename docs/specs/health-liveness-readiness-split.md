...
- **`crawl4ai.ts` owns the Crawl4AI probe.** `probeCrawl4AI(timeoutMs)` reuses the existing memoised `getClient()` and issues a cheap `tools/list` protocol call, passing `timeoutMs` as the MCP request timeout. It is exported as a purpose-named probe; the raw `Client` is **never** exported (`docs/PRODUCT.md` principle 3, "Provider details stay behind clients").
  - **Failure-state handling** — ⚠️ **superseded by the Amendment section below; read that, not this paragraph, for the live rule.** As originally drafted this said: reset `client`/`connecting` on a probe *rejection* but never on a *timeout*, because the probe had no way to close what it abandoned. Both halves changed. The amendment gives the probe a `resetClient()` that closes the transport, and the final rule resets **only for a connect-step failure** (including a connect-level timeout) or a mid-request failure that is not an `McpError`. A `tools/list` timeout or protocol error against an already-connected client must not reset, because that client is live and shared with concurrent crawl tools. Retained here only so the reasoning trail is legible; do not implement this paragraph. Do not "align" `call()` with any of it — that file belongs to a sibling story.
  - **Discriminating "timeout" from "rejection"** — this is exact, not a judgement call. An MCP request timeout surfaces as a *rejection* too (`client.listTools(..., { timeout })` rejects with an `McpError` whose code is `ErrorCode.RequestTimeout`), so "it rejected" alone is not the test. Classify as `timeout` **only** when either (a) the rejection is `err instanceof McpError && err.code === ErrorCode.RequestTimeout` (both imported from `@modelcontextprotocol/sdk/types.js`), or (b) `readiness.ts`'s own outer `Promise.race` bound fired first. Everything else — a rejected `getClient()` connect, a transport error, an unexpected throw — is a `rejection`. (⚠️ The final sentence of this bullet originally read "the `timeout` branch resets nothing; every other failure branch resets". That reset policy is **superseded by the Amendment section below**; the classification rule above still stands unchanged. Which failures reset is decided there, by *step*, not by detail.)
...
- [ ] Append `probeCrawl4AI(timeoutMs)` to `packages/toolkit/src/crawl4ai.ts`: reuse `getClient()`, bound the connect step, issue `tools/list` with the MCP request timeout, and return a safe outcome. **Reset policy is defined by the Amendment section — reset only for a connect-step failure (including a connect-level timeout) or a mid-request failure that is not an `McpError`; never for a `tools/list` timeout or protocol error against a live, shared client.** Add the transport ownership guard from the second amendment. Do not reorder or reformat existing code in that file.
