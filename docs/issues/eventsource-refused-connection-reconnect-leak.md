...
One known gap remains on a path this story did not own: `call()`'s own catch still nulls `client` / `connecting` directly without clearing `activeTransport` (documented at `packages/toolkit/src/crawl4ai.ts:46-50`), so that path can still orphan a transport and its reconnect timer. It belongs to the sibling story [`../tasks/normalize-crawl4ai-config-payloads.md`](../tasks/normalize-crawl4ai-config-payloads.md), which reduces the symptom on that call path but does not fix this upstream cause.
