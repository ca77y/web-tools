...
The one call path that formerly still leaked has since been closed. `call()`'s own catch used to null `client` / `connecting` directly without clearing `activeTransport` or closing the transport, so a failed tool request could orphan a transport and its reconnect timer. The [`crawl4ai-mcp-client-timeout-and-recovery`](../tasks/crawl4ai-mcp-client-timeout-and-recovery.md) story reworked that catch to route every connection-level discard through the same ownership-guarded `resetClient(transport)` and to guard `getClient()`'s connect step through it too, so no `call()` path abandons a transport without closing it (see [`../ARCHITECTURE.md`](../ARCHITECTURE.md) "Crawl4AI MCP Client Lifecycle"). Every toolkit path that discards the shared client now owns its teardown.
...
- `packages/toolkit/src/crawl4ai.ts` — shared state at `:7-16`, `getClient()` at `:18-102`, the transport handlers at `:84-93`, `resetClient()` at `:136`, `withConnectTimeout()` at `:189`, `probeCrawl4AI()` at `:271` with its two reset branches at `:290` and `:308`
