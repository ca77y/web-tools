...
- The timeout behavior itself — a 60s SDK default deadline sitting under a 120s+ crawl budget — was a separate, fixable defect, and it has now shipped via `crawl4ai-mcp-client-timeout-and-recovery`. Every Crawl4AI `callTool` now passes an explicit `Config.crawl4ai.callTimeoutMs` (default `150000` ms, above the toolkit's ~135 000 ms worst-case crawl budget) instead of relying on the 60s SDK default, so a legitimately slow crawl no longer times out client-side. See [`../ARCHITECTURE.md`](../ARCHITECTURE.md) "Crawl4AI MCP Client Lifecycle". This does not recover the historical correlation question above, which stays closed as unanswerable.
