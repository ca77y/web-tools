...
- [x] Split liveness from dependency readiness in health reporting #improvement ⏫ 🆔 health-liveness-readiness-split
    ...
    - Explicitly out of scope, with rationale:
        ...
        - **Standing up a test framework.** Test infrastructure now exists and this story must use it: each package has a `test` script (`tsc -p tsconfig.test.json && node --test dist-test/*.test.js`) on Node's built-in `node:test` runner, with `pnpm test` at the root and existing `*.test.ts` files beside the source, including `packages/api/src/handler.test.ts` and `packages/api/src/mcp.test.ts`. Add automated coverage for the acceptance criteria as `packages/api/src/*.test.ts` — the endpoint behaviour, per-dependency status shaping, timeout bound, cache TTL, and auth rejection are all unit-testable against stubbed dependency probes. The manual reproduction below remains the way to verify real container-level failure modes (steps 2-4 in particular), not a substitute for the tests. Do not add a test framework dependency; see `packages/CLAUDE.md`.
    ...
    - Acceptance criteria:
        ...
        - `pnpm build`, `pnpm typecheck`, and `pnpm test` pass, with the new readiness tests included in the suite.
