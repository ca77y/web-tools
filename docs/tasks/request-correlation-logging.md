...
- [ ] Add request correlation IDs and structured operation logging #improvement ⏫ 🆔 request-correlation-logging
    ...
    - Constraints:
        - `AGENTS.md` and [`../ARCHITECTURE.md`](../ARCHITECTURE.md) require provider behavior to stay in `packages/toolkit`; `packages/api` and `packages/cli` remain transport adapters. The shared logger belongs in the toolkit, and the API must not grow its own provider-facing logging.
